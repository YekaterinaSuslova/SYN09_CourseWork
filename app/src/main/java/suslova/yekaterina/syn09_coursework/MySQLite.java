...
public class MySQLite extends SQLiteOpenHelper {
    ...
    static final String DATABASE_NAME = "batteries"; // Имя базы данных

    static final String TABLE_NAME = "batteries"; // Имя таблицы
    ...
    static final String CAPACITY = "capacity"; // Поле с ёмкостью

    static final String ASSETS_FILE_NAME = "batteries.txt"; // Имя файла из ресурсов с данными для БД
    ...
    // Добавление нового аккумулятора в БД
    public void addData(SQLiteDatabase db, String name, String capacity) {
        ...
        try {
            ...
            while ((str = in.readLine()) != null) { // Читаем строку из файла
                ...
                if (!strTrim.equals("")) { // Если строка не пустая, то
                    ...
                    String capacity = st.nextToken().trim(); // Извлекаем из строки объем без пробелов на концах
                    addData(db, name, capacity); // Добавляем название и ёмкость в базу данных
                }
            }
    ...
    public String getData(String filter) {
        ...
        if (filter.equals("")) {
            ...
            selectQuery = "SELECT  * FROM " + TABLE_NAME + " WHERE (" + NAME_LC + " LIKE '%" +
                    filter.toLowerCase() + "%'" +
                    " OR " + CAPACITY + " = '" + filter + "'" + ") ORDER BY " + NAME;
        }
        ...
        if (cursor.moveToFirst()) { // Если есть хоть одна запись, то
            do { // Цикл по всем записям результата запроса
                String name = cursor.getString(1); // Чтение названия
                String capacity = cursor.getString(3); // Чтение ёмкости
                data.append(String.valueOf(++num) + ") " + name + " ёмкость в мА*ч: " + capacity + "\n");
