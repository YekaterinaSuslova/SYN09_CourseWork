...
public class ActivityTwo extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.email) {
            ...
            i.putExtra(Intent.EXTRA_SUBJECT, getString(R.string.Добавьте_еще_аккумулятор));
            i.putExtra(Intent.EXTRA_TEXT, getString(R.string.Предлагаю_такой_аккумулятор));
            try {
                startActivity(Intent.createChooser(i, getString(R.string.Посылка_письма)));
            } catch (android.content.ActivityNotFoundException ex) {
                Toast.makeText(ActivityTwo.this, R.string.Нет_установленного_почтового_клиента, Toast.LENGTH_SHORT).show();
            }
            return true;
        }
